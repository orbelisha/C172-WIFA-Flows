...
- The **Oral Exam** category follows the ACS Area/Task order of Or's own prep
  guide ("PPL Oral Exam Preparation Questions", Liraz Bershadsky, `oral_data2.py`
  header). Sub-sections are `The Checkride`, then `I.A` through `I.H`, then
  `II — Preflight Procedures`; `check_pave.mjs` asserts that ORDER, not just the
  set, because the whole point is that the app and the document he studies from
  line up Task for Task. Its content is regenerated by `oral_rebuild.py` from
  `oral_data.py` + `oral_data2.py` — edit those, not the HTML, or the next
  rebuild silently reverts you. `oral_data3.py` is the gap-fill layer: it
  appends into the EXISTING sub-sections via its `INSERTS` map rather than
  adding new ones, so the ACS Task map stays one-to-one with the guide.
  ...
  6,000 lb / 6 occupants); the current ones are 6 / 12,500 / 7, and
  `check_pave.mjs` fails if the old numbers ever come back as current.
  The category deliberately does NOT restate ARROW, AV1ATE, A TOMATO FLAMES,
  FLAPS, NWKRAFT, IMSAFE, PAVE, NOTAMTypes or SAFETY — those already have a
  parent, and one parent per topic is the rule.
